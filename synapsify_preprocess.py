...
def get_sentiment_indices(rows, sentcol):
	...
	if ((row[sentcol]=='Positive') or (row[sentcol]=='Neutral')):
		XX['pos'].append(r)
    for r,row in enumerate(rows):
	if ((row[sentcol]=='Negative') or (row[sentcol]=='Mixed')):
		XX['neg'].append(r)
    '''
    ### Syntax Error for original code
    ### Original Code: XX['pos'] = [r for r,row in enumerate(rows): if ((row[sentcol]=='Positive') | (row[sentcol]=='Neutral'))]
    ### Original Code: XX['neg'] = [r for r,row in enumerate(rows): if ((row[sentcol]=='Negative') | (row[sentcol]=='Mixed'))]
    ### Correct Code:
...
def get_rand_indices(len_set, num_indices, forbidden):
    ...
    XX = range(int(initial),int(initial)+int(num_indices))	### TypeError: unsupported operand type(s) for +: 'int' and 'str'
    if XX[-1]>len_set: print "Test/Train set indices are out of bounds!!"
    return XX	### Return the list of index of training set
...
def main(directory, filename, textcol, sentcol, train_size, test_size):
    ...
    ### textcol is the comment column, and the sentcol is the tagging column
    ...
    ### Error: Too many values to unpack, and list indices must be integers, not str
    len_sentences = len(sentences)
    DICTIONARY = build_dict(sentences)

    # TRAINING SET TRAINING SET TRAINING SET TRAINING SET ----------- train_xx : index
    ...
    XX = get_sentiment_indices(train_rows, int(sentcol))
...
if __name__ == '__main__':
    ### In argument vector: 0 should be .py file which should be run; 1 should be directory; 2 should be filename; ... 
